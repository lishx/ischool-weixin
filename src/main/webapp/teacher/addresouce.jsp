							...
							<li>
								<div class="item-content">
									<div class="item-inner">
										...
										<textarea type="text" name="lminfo" value=""></textarea>
									</div>
								</div>
							</li>
							<li><a href="">
					...
					<ul>
						<li><a href="">
								<div class="item-content">
									...
									<div class="item-inner">
										<div class="item-title-row">
											<div class="item-title col_00 fs_875"
												onclick="$('#lefile').click();">上传文件</div>
										</div>
									</div>
								</div>
						</a>
					...
					.on(
							...
							function() {
								...
								upladFile({
									fileid : "lefile",
									server : path
											+ '/index/uploadFile?systemCode=ischool&businessKey='
											+ 'material${maxId}',
									callback : function(data) {
										//$("#progress").addClass("hide");
										var dela = $("#d" + (count - 1));
										var fid = dela.attr("fileId");
										dela.attr("fileId", fid.replace(
												"{fileId}", data.fileId))
										$.alert("上传成功");
